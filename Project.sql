...
ALTER TABLE Workers
ADD CONSTRAINT Workers_FK FOREIGN KEY (WorkerID) REFERENCES People(PersonID)
ON DELETE CASCADE
ON UPDATE CASCADE


ALTER TABLE Customers
ADD CONSTRAINT Customers_FK FOREIGN KEY (CustomerID) REFERENCES People(PersonID)
ON DELETE CASCADE
ON UPDATE CASCADE
...
IF OBJECT_ID ( 'Most_expensive_worker', 'v' ) IS NOT NULL   
    DROP VIEW Most_expensive_worker;  
GO
CREATE VIEW Most_expensive_worker
AS
...
AS
	BEGIN TRY
					...
					ELSE
						...
						IF NOT EXISTS (SELECT PESEL FROM People WHERE PESEL = @pes)
						BEGIN
							INSERT INTO People VALUES (@pes, @Name, @Surname, @Birthdate)
						END

						DECLARE @WorkerID INT = (SELECT PersonID FROM People WHERE PESEL = @pes)

						IF EXISTS (SELECT WorkerID FROM Workers WHERE @WorkerID = WorkerID)
						BEGIN
							RAISERROR('Podana osoba jest ju¿ pracownikiem',16,1)
						END

						INSERT INTO Workers VALUES(@WorkerID, @Salary)
